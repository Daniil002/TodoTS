...
export const TaskCard = ({ task, onDeleteTask, onShowDelete }) => {
  ...
  return (
    <div className={style["task-card"]}>
      ...
      <div className={style["task-status-wrapper"]}>
        <button
          className={classNames(style[`status--${status}`], style["status"])}
        >
          {status}
        </button>
      </div>
      ...
      <div className={style["actions"]}>
        <EditIcon
          className={classNames(style["mr-20"], style["cp"])}
          onClick={() => {}}
        />
        <DeleteIcon className={style["cp"]} onClick={() => onShowDelete(id)} />
      </div>
    </div>
  );
};
